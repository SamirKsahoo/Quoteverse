...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* ═══════════════════════════════════════════════════════════
       MAPLIBRE POI POPUP  (hover — lightweight)
    ═══════════════════════════════════════════════════════════ */
    ...
    /* Click popup — "See distance" button inside hover popup */
    .poi-popup-dist-btn {
      margin-top: 7px; width: 100%; padding: 5px 0;
      background: #1b2a3b; color: #fff; border: none;
      border-radius: 6px; font-size: 12px; font-weight: 700;
      cursor: pointer; transition: background .2s;
    }
    .poi-popup-dist-btn:hover { background: #2d4a6b; }

    /* ═══════════════════════════════════════════════════════════
       DISTANCE PANEL  (slides up from bottom-centre)
    ═══════════════════════════════════════════════════════════ */
    .dist-panel {
      position: absolute; bottom: 28px; left: 50%; transform: translateX(-50%) translateY(20px);
    ...
    .dist-panel-poi-name {
      font-size: 14px; font-weight: 700; color: #1b2a3b; line-height: 1.3;
    }
    .dist-panel-poi-type {
      font-size: 11px; color: #64748b; text-transform: capitalize; margin-top: 2px;
    }
    .dist-panel-close {
      background: #f1f5f9; border: none; border-radius: 50%;
      width: 26px; height: 26px; font-size: 13px; cursor: pointer;
      display: flex; align-items: center; justify-content: center;
      flex-shrink: 0; color: #475569; transition: background .2s;
    }
    .dist-panel-close:hover { background: #e2e8f0; }

    .dist-rows { display: flex; flex-direction: column; gap: 8px; }
    .dist-row {
      display: flex; align-items: center; justify-content: space-between;
      background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 9px;
      padding: 9px 12px; gap: 10px; cursor: pointer;
      transition: border-color .2s, box-shadow .2s;
    }
    .dist-row:hover { border-color: #94a3b8; box-shadow: 0 2px 8px rgba(0,0,0,.07); }
    .dist-row.active { border-color: #3b82f6; box-shadow: 0 0 0 2px rgba(59,130,246,.18); background: #eff6ff; }

    .dist-row-left { display: flex; align-items: center; gap: 8px; }
    .dist-proj-dot { width: 10px; height: 10px; border-radius: 50%; flex-shrink: 0; }
    .dist-proj-name { font-size: 13px; font-weight: 600; color: #1b2a3b; }
    .dist-km {
      font-size: 13px; font-weight: 700; color: #3b82f6;
      white-space: nowrap;
    }
    .dist-row-hint { font-size: 10px; color: #94a3b8; margin-top: 1px; }

    /* Dashed route line drawn on map */
    /* (styling via MapLibre paint — no CSS needed) */
...
<div class="layout">
  ...
  <div class="dist-panel" id="distPanel">
    <div class="dist-panel-header">
      <div>
        <div class="dist-panel-poi-name" id="distPoiName">—</div>
        <div class="dist-panel-poi-type" id="distPoiType"></div>
      </div>
      <button class="dist-panel-close" id="distPanelClose" title="Close">✕</button>
    </div>
    <div class="dist-rows" id="distRows">
      {{-- filled by JS --}}
    </div>
  </div>
...
const poiPopup       = new maptilersdk.Popup({ closeButton: false, closeOnClick: false, maxWidth: '240px' });

// Distance panel state
let distActivePoi    = null;   // { lngLat, name, type } of currently selected POI
let distActiveRowCat = null;   // which project row is showing the route line
...
map.on('load', async () => {
  ...
  /* 5b. Route line source + layer (distance panel) */
  map.addSource('dist-route', { type: 'geojson', data: EMPTY_FC });
  map.addLayer({
    id: 'dist-route-casing', type: 'line', source: 'dist-route',
    paint: { 'line-color': '#fff', 'line-width': 6, 'line-opacity': 0.7 },
  });
  map.addLayer({
    id: 'dist-route-line', type: 'line', source: 'dist-route',
    paint: {
      'line-color':     '#3b82f6',
      'line-width':     3,
      'line-dasharray': [2, 3],
...
function addPoiLayers() {
  Object.entries(POI_CATS).forEach(([cat, cfg]) => {
    if (cfg.type === 'highway') {
      ...
      /* Hover popup — shows name + "See distance" button */
      map.on('mouseenter', cfg.layerId, e => {
        map.getCanvas().style.cursor = 'pointer';
        const f      = e.features[0];
        const name   = f.properties.name || 'Unnamed';
        const type   = f.properties.amenity || f.properties.shop || f.properties.aeroway || cat;
        const coords = f.geometry.coordinates.slice(); // [lng, lat]

        poiPopup
          .setLngLat(coords)
          .setHTML(`
            <div class="poi-popup-title">${name}</div>
            <div class="poi-popup-type">${type}</div>
            <button class="poi-popup-dist-btn"
              onclick="openDistancePanel(${coords[0]}, ${coords[1]}, ${JSON.stringify(name).replace(/"/g,"'")}, '${type}')">
              📏 See distance from projects
            </button>
          `)
          .addTo(map);
      });
      ...
      /* Click also opens distance panel (for touch / fast clicks) */
      map.on('click', cfg.layerId, e => {
        ...
        poiPopup.remove();
        openDistancePanel(lng, lat, name, type);
      });
    }
  });
}
 ...
 * Returns distance in km (2 decimal places as string).
 */
...
/**
 * Called when user clicks "See distance" in the hover popup OR clicks a POI dot directly.
 * Builds the distance panel rows — one per project.
 */
function openDistancePanel(poiLng, poiLat, poiName, poiType) {
  distActivePoi = { lng: poiLng, lat: poiLat, name: poiName, type: poiType };
  distActiveRowCat = null;

  // Clear any existing route line
  map.getSource('dist-route')?.setData(EMPTY_FC);

  // Populate panel header
  document.getElementById('distPoiName').textContent = poiName || 'Unnamed';
  document.getElementById('distPoiType').textContent = poiType || '';
  ...
  if (!projectMeta.length) {
    rows.innerHTML = '<div style="font-size:12px;color:#94a3b8;text-align:center">No projects loaded</div>';
  } else {
    projectMeta.forEach(proj => {
      const km  = haversine(proj.center_lat, proj.center_lng, poiLat, poiLng);
      const row = document.createElement('div');
      row.className    = 'dist-row';
      row.dataset.projId = proj.id;
      row.innerHTML = `
        <div class="dist-row-left">
          <div class="dist-proj-dot" style="background:${proj.color}"></div>
          <div>
            <div class="dist-proj-name">${proj.name}</div>
            <div class="dist-row-hint">Click to show route on map</div>
          </div>
        </div>
        <div class="dist-km">~${km} km</div>
      `;
      row.addEventListener('click', () => showRouteOnMap(proj, poiLng, poiLat, row));
      rows.appendChild(row);
    });
  }

  // Show panel
  document.getElementById('distPanel').classList.add('open');
}

/**
 * Draws a dashed line on the map between the project centre and the POI,
 * and fits the map to show both points.
 */
function showRouteOnMap(proj, poiLng, poiLat, clickedRow) {
  // Toggle: clicking the same row again hides the line
  const isSame = distActiveRowCat === proj.id;
  ...
  if (isSame) return; // second click = close

  distActiveRowCat = proj.id;
  clickedRow.classList.add('active');

  // Draw dashed line: project centre → POI
  const lineGj = {
    type: 'FeatureCollection',
    features: [{
      type: 'Feature',
      geometry: {
        type: 'LineString',
        coordinates: [
          [proj.center_lng, proj.center_lat],
          [poiLng,          poiLat],
        ],
      },
      properties: {},
  ...
  map.getSource('dist-route').setData(lineGj);

  // Fit map to show both ends with padding
  const minLng = Math.min(proj.center_lng, poiLng);
  const maxLng = Math.max(proj.center_lng, poiLng);
  const minLat = Math.min(proj.center_lat, poiLat);
  const maxLat = Math.max(proj.center_lat, poiLat);

  map.fitBounds(
    [[minLng - 0.02, minLat - 0.02], [maxLng + 0.02, maxLat + 0.02]],
    { padding: { top: 80, bottom: 180, left: 60, right: 60 }, duration: 900 }
  );
}

/** Close the distance panel and clear the route line */
function closeDistancePanel() {
  document.getElementById('distPanel').classList.remove('open');
  map.getSource('dist-route')?.setData(EMPTY_FC);
  document.querySelectorAll('.dist-row').forEach(r => r.classList.remove('active'));
  distActivePoi    = null;
  distActiveRowCat = null;
...
document.getElementById('distPanelClose').addEventListener('click', closeDistancePanel);

/* Also close the panel if the user clicks the map background */
map.on('click', e => {
  // Only close if the click wasn't on a POI layer
  const poiLayerIds = Object.values(POI_CATS)
    .filter(c => c.type !== 'highway')
    .map(c => c.layerId);
  const hit = map.queryRenderedFeatures(e.point, { layers: poiLayerIds });
  if (!hit.length) closeDistancePanel();
});
